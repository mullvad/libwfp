...
ConditionRemoteIp::ConditionRemoteIp(const IpAddress &netnode, const IStrictComparison &comparison)
	: m_net(netnode)
	, m_prefix(0)
...
ConditionRemoteIp::ConditionRemoteIp(const IpAddress &network, uint8_t prefix, const IStrictComparison &comparison)
	: m_net(network)
	, m_prefix(prefix)
	, m_comparison(comparison)
{
...
std::wstring ConditionRemoteIp::toString() const
{
	...
	switch (m_addressType)
	{
		...
		{
			ss << L"remote ip " << m_comparison.toString() << L" " << common::string::FormatIpv4(m_net.addr());
			break;
		}
		case AddressType::Ipv4Network:
		{
			ss << L"remote ip net " << m_comparison.toString() << L" " << common::string::FormatIpv4(m_net.addr(), m_prefix);
			break;
		}
		case AddressType::Ipv6:
		{
			ss << L"remote ip " << m_comparison.toString() << L" " << common::string::FormatIpv6(m_net.addr6().byteArray16);
			break;
		}
		case AddressType::Ipv6Network:
		{
			ss << L"remote ip net " << m_comparison.toString() << L" " << common::string::FormatIpv6(m_net.addr6().byteArray16, m_prefix);
			break;
		}
		default:
		{
...
const GUID &ConditionRemoteIp::identifier() const
{
	return FWPM_CONDITION_IP_REMOTE_ADDRESS;
}

const FWPM_FILTER_CONDITION0 &ConditionRemoteIp::condition() const
{
